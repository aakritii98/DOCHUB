...
router.post("/postteacherDetails", (req, res)=>{
   const data = req.body;
   console.log(data);
   Teacher.findOneAndUpdate({teacher_id:data.email},{teacher_name: data.name, teacher_email:data.emailid, phonenumber:data.phonenumber,address:data.address,eduction:data.eduction,dateofbirth:data.dateofbirth,city:data.city,pincode:data.pincode,country:data.country,state:data.state }).then((doc)=>{
       if(!doc)
       {
           console.log(err);
           res.json({
