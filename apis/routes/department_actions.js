...
router.post("/postdepartDetails", (req, res)=>{
    const data = req.body;
    console.log(data);
    Department.findOneAndUpdate({department_id:data.email},   {name: data.name, phonenumber:data.phonenumber,department_email:data.emailid,city:data.city,pincode:data.pincode,country:data.country,state:data.state }).then((doc)=>{
        if(!doc)
        {
            console.log(err);
            res.json({
